...
import random
from utils import circular_function,save_dmodel,load_dmodel
...
def main():
    ...
    if (model_type == 'SWD'):
        ...
        model_dir = os.path.join(args.outdir,
                                 model_type + '_iter' + str(args.niter) + '_n' + str(num_projection) + '_lam' + str(
                                     args.lam))
    ...
    if (dataset == 'CIFAR'):
        ...
        num_chanel = 3
        train_loader = torch.utils.data.DataLoader(
            datasets.LSUN(args.datadir + '/lsun', classes=['bedroom_train'],
                          transform=transforms.Compose([
                              transforms.Resize(64),
        ...
        test_loader = torch.utils.data.DataLoader(dataset, batch_size=1000,
                                                  shuffle=True, num_workers=args.num_workers, pin_memory=True)
    ...
    for epoch in range(epoch_cont, args.epochs):
        ...
        for batch_idx, (data, y) in tqdm(enumerate(train_loader, start=0)):
            if (model_type == 'SWD'):
                ...
                loss = model.compute_loss_GSWD(dis, disoptimizer, data, torch.randn, g_function, args.r, num_projection,
                                               p=args.p)
            elif (model_type == 'MGSWNN'):
                ...
                loss = model.compute_lossDSWD(dis, disoptimizer, data, torch.randn, num_projection, transform_net,
                                              op_trannet, p=args.p, max_iter=args.niter, lam=args.lam)
            elif (model_type == 'DGSWD'):
                loss = model.compute_lossDGSWD(dis, disoptimizer, data, torch.randn, num_projection, transform_net,
                                               op_trannet, g_function, args.r, p=args.p, max_iter=args.niter,
                                               lam=args.lam)
            optimizer.zero_grad()
